...
from mvn.datasets.mpi_inf_3dhp import get_sample_test, get_sample_train
...
# dataset loading
if not args.evaluate:
    train_dataset = wds.WebDataset(f'{args.webdataset_path}/train/'+'data-{000000..000092}.tar', shardshuffle=True)
    train_dataset = train_dataset.shuffle(1000).decode('pil').to_tuple('png', 'pyd').map(get_sample_train)
    train_dataloader = torch.utils.data.DataLoader(train_dataset.batched(config.train.batch_size), batch_size=None, 
                                                   num_workers=config.train.num_workers)
    val_dataset = wds.WebDataset(f'{args.webdataset_path}/test/data-000000.tar', shardshuffle=False)
    val_dataset = val_dataset.decode('pil').to_tuple('png', 'pyd').map(get_sample_test)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=config.val.batch_size, 
...
if not args.evaluate:
    checkpoint_conpose = torch.load('checkpoint/mpi-inf_nips_retrain.bin')['model']
...
if args.resume or args.evaluate:
    ...
    model_pos_val.load_state_dict(checkpoint, strict=False)
    ...
    while epoch < args.epochs:
        ...
        if (epoch + 1) % 5 == 0:
            with torch.no_grad():
                ...
                for images, inputs_3d, inputs_2d, inputs_2d_crop in val_dataloader:
        ...
        log_path = os.path.join(args.checkpoint, 'training_log_finetune.txt')
    ...
    with torch.no_grad():
        ...
        for images, inputs_3d, inputs_2d, inputs_2d_crop in val_dataloader:
            ...
            if args.visualize:
                ...
                np.save(f'predictions/mpi_img_{it}.npy', img)
                np.save(f'predictions/mpi_gt_{it}.npy', inputs_3d)
                np.save(f'predictions/mpi_pred_{it}.npy', predicted_3d_pos)
        ...
        np.save(f'predictions/mpi_predictions.npy', torch.vstack(pred_skeletons).numpy())
        np.save(f'predictions/mpi_gt.npy', torch.vstack(gt_skeletons).numpy())
