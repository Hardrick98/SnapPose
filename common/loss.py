...
def mpjpe(predicted, target, return_joints_err=False):
    ...
    if not return_joints_err:
        ...
        errors = torch.norm(predicted - target, dim=len(target.shape)-1)  # errors: [B, T, N]
...
def mpjpe_diffusion_median(predicted, target):
    """
    Mean per-joint position error (i.e. mean Euclidean distance),
    often referred to as "Protocol #1" in many papers.
    """
...
def p_mpjpe_diffusion_median(predicted, target):
    """
    Mean per-joint position error (i.e. mean Euclidean distance),
    often referred to as "Protocol #1" in many papers.
    """
    b_sz, t_sz, h_sz, f_sz, j_sz, c_sz = predicted.shape
    predicted = torch.median(predicted, dim=2, keepdim=False)[0]
